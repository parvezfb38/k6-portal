...
app.post('/api/run-test', upload.single('file'), (req, res) => {
  let script;
  ...
  exec(`k6 run --out json=results.json test-script.js`, (error, stdout, stderr) => {
    // K6 returns exit code 99 when thresholds are crossed, but test still completes
    ...
    // Parse both console output and JSON results
    const metrics = parseK6Metrics(stdout);
    const urlMetrics = parseUrlMetrics();
    
    // Include threshold warning if exit code was 99
